...
export const NbReconversionRHField = chakra(({ disabled, className }: { disabled?: boolean; className?: string }) => {
  const {
    formState: { errors },
    control,
    watch,
  ...
  return (
    <FormControl className={className} isInvalid={!!errors.nbReconversionRH}>
      ...
      />
      {errors.nbReconversionRH && <FormErrorMessage>{errors.nbReconversionRH.message}</FormErrorMessage>}
    </FormControl>
  );
});
