...
const useEtablissementsTab = ({
  ...
  const { data: dataEtablissementsMap, isLoading: isLoadingEtablissements } =
    client.ref("[GET]/formation/:cfd/map").useQuery(
      {
        params: { cfd },
        query: {
          codeRegion,
  ...
  return (
    <Flex direction={"column"} gap={8} w={"60%"}>
      ...
      {etabFilters.view === "list" && (
        <List isLoading={isLoading} etablissements={etablissements} />
      )}
    </Flex>
  );
};
