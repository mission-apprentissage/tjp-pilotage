...
import { Filters, TauxAttractiviteType, TauxIJType } from "../../../../types";
...
const useIndicateursTab = ({
  ...
  const [tauxIJSelected, setTauxIJSelected] = useState<TauxIJType>(
    "tauxDevenirFavorable"
  );
  const [tauxAttractiviteSelected, setTauxAttractiviteSelected] =
    useState<TauxAttractiviteType>("tauxPression");
  const { data: dataFormation, isLoading: isLoadingFormation } = client
    .ref("[GET]/formation/:cfd")
    .useQuery(
      {
        params: { cfd },
  ...
  const { data: dataIndicateurs, isLoading: isLoadingIndicateurs } = client
    .ref("[GET]/formation/:cfd/indicateurs")
    .useQuery(
      {
        params: { cfd },
        query: { codeRegion, codeAcademie, codeDepartement },
      ...
      {dataFormation.isInScope ? (
        <>
          ...
          <Flex direction={"column"} gap={4}>
            ...
            <SoldeDePlacesTransformeesCard
              scope={scope}
              data={dataIndicateurs}
            />
          </Flex>
