import { FormationIndicateurs, TauxIJType } from "../../../../types";

export const displaySoldeDePlacesTransformees = (
  data: FormationIndicateurs
): boolean => {
  return (
    data?.soldePlacesTransformee !== undefined &&
    data?.soldePlacesTransformee.length > 0
  );
};

export const displayIJDatas = (
  data: FormationIndicateurs,
  ...
  return (
    ...
    data.tauxIJ[tauxIJSelected].some(
      (data) =>
        typeof data.apprentissage !== "undefined" ||
        typeof data.scolaire !== "undefined"
    )
  );
};
...
export const displayTauxAttractiviteDatas = (
