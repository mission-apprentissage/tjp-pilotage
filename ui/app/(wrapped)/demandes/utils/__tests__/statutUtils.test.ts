...
import {getPossibleNextStatuts} from '@/app/(wrapped)/demandes/utils/statutUtils';
...
const fixtureBuilder = () => {
  ...
  let possibleNextStatuts: Array<DemandeStatutType> = [];
  ...
  return {
    ...
    then: {
      verifierNextStatuts: (possibleNextStatutsValue: Array<DemandeStatutType>) => {
        expect(possibleNextStatuts.sort((a,b) => a.localeCompare(b)))
          .toEqual(possibleNextStatutsValue.sort((a,b) => a.localeCompare(b)));
      }
    },
  };
};
...
describe("ui > app > (wrapped) > demandes > utils > statutUtils", () => {
