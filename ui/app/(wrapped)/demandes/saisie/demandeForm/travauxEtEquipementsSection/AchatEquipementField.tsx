...
export const AchatEquipementField = chakra(({ disabled, className }: { disabled?: boolean; className?: string }) => {
  const {
    formState: { errors },
    control,
  ...
  return (
    <FormControl as="fieldset" className={className} isInvalid={!!errors.achatEquipement} isRequired>
      <FormLabel as="legend">Votre proposition s'accompagne-t-elle d'achat d'équipements ?</FormLabel>
      <Controller
      ...
      />
      {errors.achatEquipement && <FormErrorMessage>{errors.achatEquipement?.message}</FormErrorMessage>}
    </FormControl>
  );
});
