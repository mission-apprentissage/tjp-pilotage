...
export const DemandeFilesContext = createContext<DemandeFilesContext>({} as DemandeFilesContext);

const useGetFiles = (numero: string | undefined) => {
  const { isLoading: isLoadingFiles, data: uploadedFiles } = client
    .ref("[GET]/demande/:numero/files")
...
const useDeleteFiles = (numberOfFiles: number) => {
  ...
  const { isLoading: isDeletingFiles, mutateAsync: deleteFiles } = client
    .ref("[DELETE]/demande/:numero/files")
    .useMutation({
      onSuccess: () => {
        const toastContent: UseToastOptions = {
          status: "success",
...
export const DemandeFilesProvider = ({ children, numero }: { readonly children: ReactNode; numero?: string }) => {
  const toast = useToast();
  ...
  const downloadFile = async (file: FileType) => {
    try {
      ...
      if (numero && file.isUploaded) {
        const { url } = await client.ref("[GET]/demande/:numero/files/url").query({
  ...
  return <DemandeFilesContext.Provider value={value}>{children}</DemandeFilesContext.Provider>;
};

export const useDemandeFilesContext = () => useContext(DemandeFilesContext);
