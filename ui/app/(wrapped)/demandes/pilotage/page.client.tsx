...
export const PageClient = () => {
  ...
  const setOrder = (order: OrderPilotage) => {
    setSearchParams({
      ...searchParams,
      order
    });
  ...
  const setOrderFormations = (orderFormations: OrderFormationsPilotage) => {
    setSearchParams({
      ...searchParams,
      orderFormations
    });
  ...
  const { data, isLoading } = client.ref("[GET]/pilotage").useQuery(
    {
      query: { ...filters, ...order },
    }
  );

  const { data: { formations, stats } = {} } = client.ref("[GET]/pilotage/formations").useQuery(
    {
