...
const hasPermissionModificationAvis = ({ user, avis }:{ user?: UserType, avis: Avis }) => {
  if (hasPermission(user?.role, PermissionEnum["demande-avis/ecriture"])) {
...
export const AvisSection = chakra(({ avis, statut }: { avis: Avis; statut: DemandeStatutType }) => {
  ...
  const { isLoading: isDeleting, mutateAsync: submitDeleteAvisCommentaire } = useMutation({
    mutationFn: async () => {
      await client.ref("[DELETE]/demande/avis/:id").query({
        params: {
          id: avis.id,
        },
      });
