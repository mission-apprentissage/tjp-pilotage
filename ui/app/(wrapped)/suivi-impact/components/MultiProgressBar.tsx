import { Box, Text, Tooltip, VisuallyHidden } from "@chakra-ui/react";
...
export const MultiProgressBar = ({
  ...
  return (
    <Box width="100%" position="relative" overflow="hidden" ref={containerRef}>
      <VisuallyHidden>
        {
          bars.map(({label, value}) =>(<Text key={label}>{label} - {formatPercentageFixedDigits(value, 1, "-")} soit {(value/max)*100}% de l'objectif</Text>))
        }
      </VisuallyHidden>
      {/* Zone des labels */}
        ...
        {bars.map((bar) => {
          ...
          return (
            <Box
              ...
              whiteSpace="nowrap"
              zIndex={bar.order}
              aria-label={`${bar.label} - ${formatPercentageFixedDigits(bar.value, 1, "-")}`}
            >
