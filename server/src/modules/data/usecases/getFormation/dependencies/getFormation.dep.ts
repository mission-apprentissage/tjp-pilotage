...
export const getFormation = async ({
  ...
  const listOfCfdWhichAreTransitionNumerique = kdb
    .selectFrom(
      kdb
        ...
        .select((sb) => [
          ...
          sb
            .ref("niveauDiplome.libelleNiveauDiplome")
            .as("libelleNiveauDiplome"),
          sb.ref("formationView.libelleFormation").as("libelleFormation"),
          sql<number>`count(distinct ${sb.ref("formationRome.codeRome")})`.as(
            "nb romes total"
          ),
  ...
  const listOfCfdWhichAreTransitionEcologique = kdb
    .selectFrom(
      kdb
        .selectFrom("formationView")
        .innerJoin(
  ...
  const listOfCfdWhichAreTransitionDemographique = kdb
    .selectFrom(
      kdb
        .selectFrom("formationView")
        .innerJoin(
  ...
  return kdb
    ...
    .with(
      "inTransitionDemographique",
      () => listOfCfdWhichAreTransitionDemographique
    )
    .selectFrom("formation")
    .leftJoin(
      "inTransitionNumerique",
      "formation.cfd",
      "inTransitionNumerique.cfd"
    )
    .leftJoin(
      "inTransitionEcologique",
      "formation.cfd",
      "inTransitionEcologique.cfd"
    )
    .leftJoin(
      "inTransitionDemographique",
      "formation.cfd",
      "inTransitionDemographique.cfd"
    )
    .leftJoin(
      "formation_maille_etab",
      "formation.cfd",
      "formation_maille_etab.cfd"
    )
    ...
    .select((sb) => [
      ...
      sb
        .case()
        .when("formation.cfd", "in", (sb) =>
          sb.selectFrom("inTransitionNumerique").select("cfd")
        )
        .then(true)
        .else(false)
        .end()
      ...
      sb
        .case()
        .when("formation.cfd", "in", (sb) =>
          sb.selectFrom("inTransitionEcologique").select("cfd")
        )
        .then(true)
        .else(false)
        .end()
      ...
      sb
        .case()
        .when("formation.cfd", "in", (sb) =>
          sb.selectFrom("inTransitionDemographique").select("cfd")
        )
        .then(true)
        .else(false)
        .end()
      ...
      )} = 'apprentissage' OR ${sb.ref(
        "formation_maille_etab.voie"
      )} IS NULL)`.as("isApprentissage"),
      sql<boolean>`bool_or(${sb.ref(
        "formation_maille_etab.voie"
      )} = 'scolaire' OR ${sb.ref("formation_maille_etab.voie")} IS NULL)`.as(
        "isScolaire"
      ),
      sb
        .case()
