...
import type { getFormationIndicateursSchema } from "@/modules/data/usecases/getFormationIndicateurs/getFormationIndicateurs.schema";
import type { Server } from "@/server/server";
import createServer from "@/server/server.js";

usePg();

type Response = z.infer<(typeof getFormationIndicateursSchema.response)[200]>;

describe("GET /api/formation/:cfd/indicateurs", () => {
  const cfd = "40031213";
  let app: Server;
  ...
  it("doit retourner une erreur 404 si le cfd n'existe pas", async () => {
    const response = await app.inject({
      method: "GET",
      url: "/api/formation/1234567890/indicateurs",
    });
  ...
  describe("Nombre d'établissements", () => {
    it("doit retourner le nombre d'établissement nationaux", async () => {
      const response = await app.inject({
        method: "GET",
        url: `/api/formation/${cfd}/indicateurs`,
      });
    ...
    it("doit retourner le nombre d'établissement regionaux (84)", async () => {
      ...
      const response = await app.inject({
        method: "GET",
        url: `/api/formation/${cfd}/indicateurs?codeRegion=${codeRegion}`,
      });
    ...
    it("doit retourner le nombre d'établissement du département (10)", async () => {
      ...
      const response = await app.inject({
        method: "GET",
        url: `/api/formation/${cfd}/indicateurs?codeRegion=${codeRegion}&codeAcademie=${codeAcademie}`,
      });
    ...
    it("doit retourner le nombre d'établissement du département (069)", async () => {
      ...
      const response = await app.inject({
        method: "GET",
        url: `/api/formation/${cfd}/indicateurs?codeRegion=${codeRegion}&codeAcademie=${codeAcademie}&codeDepartement=${codeDepartement}`,
      });
