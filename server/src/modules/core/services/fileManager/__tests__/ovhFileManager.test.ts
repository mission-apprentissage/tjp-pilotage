...
describe("Ovh file manager", () => {
  ...
  // it("should upload a file to server directory", async () => {
  //   // given
  ...
  //   // then
  //   fileFixture.thenFileShouldBeUploaded(id, [`${id}/test.txt`]);
  // });
  ...
  //   // then
  //   expect(files).toEqual(["test-1.txt", "test.txt"]);
  // });
  ...
  //   const id = fileFixture.givenAnIntentionId();
  //   const fileBuffer = fileFixture.givenASimpleFileAsBuffer();
  //   await fileFixture.whenUploadingAFileAsBuffer(id, "test-1.txt", fileBuffer);

  //   // when
  //   await fileFixture.thenDeleteFile(id, "test-1.txt");
