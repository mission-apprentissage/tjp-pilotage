...
export const up = async (db: Kysely<DB>) => {
  ...
  await db
    .deleteFrom("rawData")
    .where("type", "like", "Cab-nbre_division_effectifs_par_etab_mefst11%")
    .execute();
  // Suppression des données des anciens fichiers d'attractivité BTS, pour cause
  // de renommage
  await db
    .deleteFrom("rawData")
    .where("type", "like", "attractivite_capacite_BTS%")
    .execute();
  ...
  await db
    ...
    .where((eb) =>
      eb.and([
        eb.or([
          eb("rentreeScolaire", "=", "2020"),
          eb("rentreeScolaire", "=", "2021"),
        ]),
        eb(
          ...
          db
            ...
            .select("id")
            .where("formationEtablissement.cfd", "like", "320%")
        ),
      ])
    )
    .execute();
