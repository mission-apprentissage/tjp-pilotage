...
export const up = async (db: Kysely<unknown>) => {
  await db.schema
    ...
    .addColumn("id", "uuid", (c) =>
      c.primaryKey().defaultTo(db.fn("uuid_generate_v4"))
    )
  ...
  await db.schema
    ...
    .addUniqueConstraint("intentionAccessLog_unique", [
      "intentionNumero",
      "userId",
    ])
    .execute();
  ...
  await db.schema
    .alterTable("intentionAccessLog")
    .addForeignKeyConstraint("fk_intentionAccessLog_user", ["userId"], "user", [
      "id",
    ])
    .execute();
};
